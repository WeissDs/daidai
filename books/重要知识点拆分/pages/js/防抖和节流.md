...
```javascript
  ...
  debounceFn: debounce(function(e){
    let event = e.target || e.srcElement;
    // console.log(111111111, event, event.scrollHeight, event.scrollTop + event.clientHeight);
    if (event.scrollHeight <= event.scrollTop + event.clientHeight + 10){
      this.haveRead = true;
    }
  }, 2000),
