...
slice()
splice()
indexOf()和 lastIndexOf() （ES6新增）
forEach() （ES6新增）
map() （ES6新增）
filter() （ES6新增）
every() （ES6新增）
some() （ES6新增）
reduce()和 reduceRight() （ES6新增）
flat() （ES6新增 平铺数组）
```
...
将数组平铺，默认传值flat(1)---只平铺一层, flat(2)---平铺两层，传参可接收数字和字符串
...
####对象数组去重
	...
	let obj = [{id:'123'},{id:'333'},{id:'666'},{id:'666'},{id:'123'},{id: '777'}];
	let result = []
	let a = []
	...
	console.log("oneArr", unique(obj, "id"));